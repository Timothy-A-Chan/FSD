        ...
        if not students:
            ...
            for student in students:
                if student.grade in ["HD", "D", "C", "P"]:
                    pass_students.append(student)  # Add to pass list
                elif student.grade in ["Z", "N/A"]:
                    fail_students.append(student)  # Add to fail list
            
            print("FAIL -->", end=" ")
            for student in fail_students:
                print(f"{student.name} :: {str(student.id).zfill(6)} --> Grade: {student.grade} Mark: {student.mark:.2f}", end=" ")
            print()  # New line after FAIL students

            print("PASS -->", end=" ")
            for student in pass_students:
                print(f"{student.name} :: {str(student.id).zfill(6)} --> Grade: {student.grade} Mark: {student.mark:.2f}", end=" ")
            print()  # New line after PASS students
